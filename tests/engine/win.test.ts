...
describe('settleWin — spec §6.2 worked examples (4 tai, base 8)', () => {
  ...
  it('discard, shooter FULL: discarder funds all 4×', () => {
    expect(settleWin(win({}), FULL)).toEqual({ E: 32, S: -32, W: 0, N: 0 });
  });
  ...
  it('self-draw: all three pay 2×, winner +6× (shooter irrelevant)', () => {
    ...
    expect(settleWin(win({ winKind: 'self_draw', discarder: undefined }), FULL)).toEqual(expected);
  });
